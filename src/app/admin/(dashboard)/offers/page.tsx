...
export default function OffersAdminPage() {
  ...
  const openCreate = () => { setEditing(null); setForm({ title: "", description: "", bannerUrl: "", ctaText: "Learn More", ctaLink: "", isActive: true, sortOrder: 0 }); setShowModal(true); };
  const openEdit = (o: Offer) => { setEditing(o); setForm({ title: o.title, description: o.description, bannerUrl: o.bannerUrl, ctaText: o.ctaText, ctaLink: o.ctaLink, isActive: o.isActive, sortOrder: o.sortOrder }); setShowModal(true); };
    ...
    <div className="flex flex-col flex-1">
      ...
      <main className="flex-1 p-6">
        <div className="flex items-center justify-between mb-6">
          <div><h2 className="text-xl font-bold text-gray-800">Manage Offers</h2><p className="text-sm text-gray-500">{offers.length} offers</p></div>
        ...
        {loading ? <div className="flex items-center justify-center h-40"><Loader2 size={32} className="animate-spin text-[#25468F]" /></div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <button onClick={() => handleToggle(o)} className="flex items-center gap-1 mt-1">
                      {o.isActive ? <><CheckCircle size={13} className="text-green-500" /><span className="text-xs text-green-600">Active</span></> : <><XCircle size={13} className="text-gray-400" /><span className="text-xs text-gray-400">Inactive</span></>}
                    </button>
                  </div>
            ...
            {offers.length === 0 && <div className="col-span-3 bg-white rounded-2xl border border-gray-100 p-12 text-center text-gray-400 text-sm">No offers yet</div>}
          </div>
        )}
      </main>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col z-10">
            <div className="p-5 border-b border-gray-100"><h2 className="font-bold text-lg">{editing ? "Edit Offer" : "Add Offer"}</h2></div>
            <form onSubmit={handleSave} className="p-5 space-y-4">
              ...
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">Description *</label><textarea className={`${ic} h-24 resize-none`} value={form.description} onChange={e => setForm({...form, description: e.target.value})} required /></div>
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">Banner Image URL</label><input className={ic} value={form.bannerUrl} onChange={e => setForm({...form, bannerUrl: e.target.value})} /></div>
